...
func _ready():
	...
	sprite_estrellas = $AnimatedSprite_estrellas
...
func _process(delta):
	...
	if Input.is_action_pressed("salto_b") and is_on_floor() and !caida_salto and salto_largo_activado:
		mostrar_puntos()
		apuntando = true
		$Flecha.visible = true
		zoom = true
	...
	if Input.is_action_just_released("salto_b") and is_on_floor()  and salto_largo_activado:
		if !caida_salto:
...
#	if direccion.x == 0 and !apuntando:
	if direccion.x == 4.5 and !apuntando:
#		print("PUSH")
...
#	print(direccion.x)
	...
	camara.global_position.y = lerp(camara.global_position.y, self.global_position.y - 80, 0.05)
...
func _on_Area2D_area_entered(area):
	...
	if area.is_in_group("tutorial_2"):
		...
		salto_largo_activado = true
		area.queue_free()
		get_tree().paused = true
	if area.is_in_group("inicio"):
		sistema_comentarios.mostrando_comentarios = true
		sistema_comentarios.mostrar_comentario("inicio")
		area.queue_free()
		get_tree().paused = true
	if area.is_in_group("enemigo_comentario"):
		sistema_comentarios.mostrando_comentarios = true
...
func _on_Area2D_area_exited(area):
	if area.is_in_group("aviso_estrellas"):
			sprite_estrellas.visible = false
